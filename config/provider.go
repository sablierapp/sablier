...
func (provider Provider) IsValid() error {
	validProviders := []string{"docker", "docker_swarm", "swarm", "kubernetes"}
	for _, p := range validProviders {
		if p == provider.Name {
			return nil
		}
	}
	return fmt.Errorf("unrecognized provider %s. providers available: %v", provider.Name, validProviders)
}
