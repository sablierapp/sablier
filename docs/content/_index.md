...
Sablier is a **free** and **open-source** API that starts your workloads on demand and stops them when there's no activity. Your workloads can be a Docker container, a Kubernetes deployment, and [more](/providers/).

It integrates with multiple reverse proxies and offers different loading [strategies](/strategies/), so you can start containers on demand and shut them down automatically once they're idle.

New to the terminology? See the [Glossary](/glossary/) for provider-agnostic definitions of session, instance, provider, group and strategy.
