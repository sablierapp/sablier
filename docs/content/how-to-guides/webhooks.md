...
Because Sablier sits in front of every supported provider (Docker, Docker Swarm, Kubernetes, Podman, Proxmox LXC, systemd, Nomad), webhooks act as a **unified, provider-agnostic event stream**: your receiver always gets the same payload structure regardless of the underlying runtime.
