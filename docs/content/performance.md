...
The first request triggers the container wakeup (≈ 1 s with mimic); subsequent
...
> The wide spread (p50 = 822 µs, p90 = 5.02 s) is expected: with only 3
...
SABLIER_IMAGE=sablierapp/sablier:v1.8.0 make bench   # pin a version
```
