...
In addition to CPU and memory, the **Docker** provider can throttle **block I/O** in [scale mode](/features/scale-mode/). This is useful when an idle workload should keep running but must not compete with active workloads for disk bandwidth.

{{< callout type="info" >}}
Block I/O throttling is currently **Docker only**. Docker Swarm, Kubernetes and Podman ignore these labels — use the badges above to request support for your provider.
{{< /callout >}}
