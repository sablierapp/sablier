...
type Instance struct {
	...
	Provider        string                          `jsonschema:"description=Provider type (docker|swarm|kubernetes|podman|proxmox_lxc|systemd|nomad),example=docker"`
	...
	Nomad           *sablier.NomadTaskGroupInfo     `json:",omitempty" jsonschema:"description=Nomad-specific task group metadata (only set when Provider is nomad). Fields are accessible in templates as .Nomad.Namespace .Nomad.JobID .Nomad.TaskGroup .Nomad.Meta"`
}
