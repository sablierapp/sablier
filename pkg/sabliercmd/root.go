...
It provides integrations with multiple reverse proxies and different loading strategies.`,
	...
	startCmd.Flags().BoolVar(&conf.Provider.StrictLabels, "provider.strict-labels", false, "Strictly require sablier.enable=true label on instance operations")
	_ = viper.BindPFlag("provider.strict-labels", startCmd.Flags().Lookup("provider.strict-labels"))
