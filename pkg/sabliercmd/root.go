...
It provides integrations with multiple reverse proxies and different loading strategies.`,
	...
	startCmd.Flags().BoolVar(&conf.Provider.IgnoreUnlabeled, "provider.ignore-unlabeled", false, "Ignore instances without sablier.enable=true during start, stop, and event operations")
	_ = viper.BindPFlag("provider.ignore-unlabeled", startCmd.Flags().Lookup("provider.ignore-unlabeled"))
